 ...
 *   RN  Average for several sizes of loop, but it's a suspiciously round 
 *       number.  This times(2) was counting whole seconds.
...
/*#define LOOPS 50000000         /* Use this for a 2010 vinatge Intel PC */
...
RecordType      xxx, yyy;
RecordPtr       PtrGlb = &xxx;
RecordPtr       PtrGlbNext=&yyy;
...
Proc0()
{
        ...
        times(tms); starttime = 0;
        for  (bb = 3; bb >= 0;  bb--)
                starttime = (starttime << 8) + tms[bb];
        for (i = 0; i < LOOPS; ++i);
        times(tms);
        nulltime = 0;
        for  (bb = 3; bb >= 0;  bb--)
                nulltime = (nulltime << 8) + tms[bb];
        nulltime = nulltime - starttime; /* Computes overhead of looping */
#endif
...
/*
        iprintf("Dhrystone(%s) time for %d passes = %d\n",
                Version,
                (long) LOOPS, benchtime);
        iprintf("This machine benchmarks at %d dhrystones/second\n",
                ((long) LOOPS) / benchtime);
*/
#endif
#ifdef TIMES
        ...
        benchtime = 0;
        for  (bb = 3; bb >= 0;  bb--)
                benchtime = (benchtime << 8) + tms[bb];
        benchtime = benchtime - starttime - nulltime;
/*
        iprintf("Dhrystone(%s) time for %d passes = %d\n",
                Version,
